...
import { IMeForProfile, IUserForProfile } from "../../types/types";

export function UserProfilePage() {
	const { user, token } = useAuthContext();
	const { posts, fetchPosts } = useFetchPosts(user?.id);

	const [userProfile, setUserProfile] = useState<IMeForProfile | null>(
		null
	);


	async function getMeById() {
		const response = await fetch(`http://${HOST}/user/me/profile`, {
			headers: {
			...
			<View style={styles.header}>
				...
				<View style={styles.statsContainer}>
					<View style={styles.statBlock}>
						<Text style={styles.statNumber}>{userProfile?.postsAmount}</Text>
						<Text style={styles.statLabel}>Дописи</Text>
					</View>
					<View style={styles.statBlock}>
						<Text style={styles.statNumber}>{userProfile?.friendAmount}</Text>
